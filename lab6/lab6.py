...
class Account:
    def __init__(self, account_number, balance, owner):
        ...
        self.__transactionShowList = []

    def __str__(self):
        return '\n'.join(self.__transactionShowList)
    ...
    def deposit(self, atm, money):
        if money > 0 and atm.get_limit() >= money:
            result1 = f"{self.get_owner().get_name()} account before test : {self.get_balance()}"
            self.__balance += money
            atm.set_limit = money
            atm.set_money_remaining_deposit = money

            transaction_show_list_detail = f"{self.get_owner().get_name()} transaction : D-ATM:{atm.get_atm_number()}-{money}-{self.__balance}"
            self.__transactionShowList.append(transaction_show_list_detail)
            result2 = f"{self.get_owner().get_name()} account after test : {self.get_balance()}"
            
            self.__transactionList.append(Transaction('Deposit', money, None, atm.get_atm_number(), None))
            
            return f"{result1}\n{result2}"
        return "Error"
    #Todo 4
    def withdraw(self, atm, money):
        if money > 0 and self.get_balance() >= money and atm.get_limit() >= money and atm.get_money_remaining() >= money:
            result1 = f"{self.get_owner().get_name()} account before test : {self.get_balance()}"
            self.__balance -= money
            atm.set_limit = money
            atm.set_money_remaining_withdraw = money
            
            transaction_show_list_detail = f"{self.get_owner().get_name()} transaction : W-ATM:{atm.get_atm_number()}-{money}-{self.__balance}"
            self.__transactionShowList.append(transaction_show_list_detail)
            result2 = f"{self.get_owner().get_name()} account after test : {self.get_balance()}" 
            self.__transactionList.append(Transaction('Withdraw', money, None, atm.get_atm_number(), None))
        
            return f"{result1}\n{result2}"
        return "Error"
    #Todo 5
    def transfer(self, atm, money, partner):
        if money > 0 and self.get_balance() >= money:
            result1 = F"{self.get_owner().get_name()} account before test : {self.__balance}"
            self.__balance -= money
            atm.set_limit = money
            self.__transactionShowList.append(F"{self.get_owner().get_name()} transaction : T-ATM:{atm.get_atm_number()}-{money}-{self.__balance}")
            result2 = F"{self.get_owner().get_name()} account after test : {self.__balance}"
            
            result3 = F"{partner.get_owner().get_name()} account before test : {partner.__balance}"
            partner.__balance += money
            partner.__transactionShowList.append(F"{partner.get_owner().get_name()} transaction : T-ATM:{atm.get_atm_number()}-+{money}-{partner.__balance}")
            result4 = F"{partner.get_owner().get_name()} account after test : {partner.__balance}"
            
            self.__transactionList.append(Transaction('Transfer', money, None, atm.get_atm_number(), self.__account_number))
...
class ATMmachine:
    ...
    def set_limit(self, money):
        if self.__limit >= money:
            self.__limit -= money
            
    def set_money_remaining_deposit(self, money):
        self.__money_remaining += money_remaining
            
    def set_money_remaaining_withdraw(self, money):
    ...
    def insert_card(self, ATMcard, pin_number):
        if ATMcard.get_pin() == pin_number:
            return ATMcard.get_atm_number(), ATMcard.get_account().get_account_number(), "Success"
        return "Error"
...
for atm_number, money_remaining in atms.items():
    bank.add_atm_list(ATMmachine(atm_number, money_remaining))
